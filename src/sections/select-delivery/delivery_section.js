  ...
  if (isEmpty(itemCount?.valid) && isEmpty(itemCount?.invalid)) {
    return (
      <HelperBar type="warning">
        ...
        <HelperBar.Button
          onClick={() => router.push("/subscription/get-started/select-plan")}
        >
          Select Plan
        </HelperBar.Button>
      </HelperBar>
      ...
      <DeliveryDays>
        {itemCount?.valid?.map((day) => (
          <DeliveryDay
            ...
            className={`${state.delivery.selected?.id === day.id && "active"}`}
          >
            <DeliveryDayLeft>
              <CheckIcon
                size={18}
                  ...
                  <Fulfillment>
                    <span>
                      {day.zipcodes[0].isDeliveryActive ? (
                        ...
                        <CrossIcon size={16} tw="stroke-current text-red-600" />
                      )}
                    </span>
                    <p>
