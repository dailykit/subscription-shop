  ...
  return (
    <section tw="px-6 w-full md:w-5/12">
      ...
      <Form.Field tw="mr-3">
        ...
        <Form.DisabledText>{user?.platform_customer?.email}</Form.DisabledText>
      </Form.Field>
    ...
    onCompleted: (data) => {
      if (data?.subscription_subscription?.length) {
        ...
        setPlan({
          ...
          servings:
            fetchedPlan.subscriptionItemCount.subscriptionServing.servingSize,
        });
      }
    },
    onError: (error) => {
            ...
            <div>
              <p tw="mb-2 text-gray-500">
                ...
                <span tw="text-gray-800">
                  {moment(user.pausePeriod.startDate).format("MMM Do YYYY")}
                </span>{" "}
                and ends on{" "}
                <span tw="text-gray-800">
                  {moment(user.pausePeriod.endDate).format("MMM Do YYYY")}
                </span>
                .{" "}
              </p>
